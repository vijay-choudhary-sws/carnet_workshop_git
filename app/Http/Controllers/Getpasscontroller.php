...
// echo "<pre>";print_r(isAdmin(Auth::User()->role_id));die;
		$currentUser = User::where([['soft_delete', 0], ['id', '=', Auth::User()->id]])->orderBy('id', 'DESC')->first();
		// $adminCurrentBranch = BranchSetting::where('id', '=', 1)->first();
		if (isAdmin(Auth::User()->role_id)) {
	...
	public function store(Request $request)
	{
		...
		$gatepass->save();
		return redirect('/gatepass/list')->with('message', 'Gatepass Created Successfully');
	}
	...
	public function gatepassprint(Request $request)
	{
		...
		return view('gatepass.getpassmodel_print')->with(compact('page_action', 'getpassid', 'getpassdata', 'setting', 'vehicle', 'job'))->render();
	}
}
