...
class Accessrightscontroller extends Controller
{
    ...
    public function storeAccessRights(Request $request)
    {
        ...
        if (!empty($accessory)){
            $data .= $this->store_regex($accessory) . ',';
        }
        if (!empty($tools)){
            $data .= $this->store_regex($tools) . ',';
        }
        if (!empty($parts)){
            $data .= $this->store_regex($parts) . ',';
        }
        if (!empty($lubricants)){
            $data .= $this->store_regex($lubricants) . ',';
        }
        if (!empty($units)){
            $data .= $this->store_regex($units) . ',';
        }
        if (!empty($categories)){
            $data .= $this->store_regex($categories) . ',';
        }
        if (!empty($order)){
            $data .= $this->store_regex($order) . ',';
        }
